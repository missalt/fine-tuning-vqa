...
class VisualEmbedding(nn.Module):
    ...
    def __init__(self, config, obj_order_embedding=None, 
        ...
        if not self.individual_vis_layer_norm and self.use_vis_layer_norm:
            self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)

    
    def forward(self, feats, img_order_ids=None, obj_order_ids=None):
        """
        Args:
            feats: [B, N, feat_dim]
        Return:
            feat_embedding: [B, N, d_model]
        """
        feat_embedding = self.feat_embedding(feats)

        if not self.individual_vis_layer_norm and self.use_vis_layer_norm:
            feat_embedding = self.layer_norm(feat_embedding)
